...
exports.parse = function parse(source){
    ...
    var instrumentNodes = function(node, parent, prev, next){
        ...
        var nextToken = next? next.startToken : (parent? parent.endToken : null);
...
function instrumentTokens(ast, source){
    ...
    while (token = tokens[++i]) {
        if (i) {
            if (prev.range[1] < token.range[0]) {
                ...
                var wsTokens = getWhiteSpaceTokens(str, prev);
                // faster then concat or push.apply
                var k = -1, ws;
                while (ws = wsTokens[++k]) {
                    result[result.length] = ws;
                }
            }
        }
        result[result.length] = prev = token;
    }
    ...
    if (lastToken.range[1] < source.length) {
        ...
        result = result.concat( getWhiteSpaceTokens(raw, lastToken) );
    }
...
function recursiveWalk(node, fn, parent, prev, next){
    ...
    while (key = keys[++i]) {
        ...
        if (typeof child.type === 'string') {
            ...
            var k = -1, c;
            while (c = child[++k]) {
                recursiveWalk(c, fn, node, (k? child[k - 1] : undefined), child[k + 1] );
            }
        }
